  ...
  try {
    ...
    for (const fetchUrl of attempts) {
      ...
      const pl = res.data.paragraphList
        ...
        || null;
      if (pl) { paragraphList = pl; if (!debug) break; }

      const entries = res.data.list || res.data.items || (Array.isArray(res.data) ? res.data : []);
      if (entries.length && entries[0].paragraphList) {
        paragraphList = entries[0].paragraphList;
        if (!debug) break;
      }
    }

    if (debug) {
      return new Response(JSON.stringify({ _debug: true, title, attempts: results, hasParagraphList: !!paragraphList }, null, 2), { headers: corsHeaders });
    }
